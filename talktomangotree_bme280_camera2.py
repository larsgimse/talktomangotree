...
def tweet_check():
    ...
    if "#image" in tweet.split():
        file = open(IMAGE_PATH, 'rb')
        data = file.read()
        r = api.request('media/upload', None, {'media': data})
        print('UPLOAD MEDIA SUCCESS' if r.status_code == 200 else 'UPLOAD MEDIA FAILURE: ' + r.text)
        tweet_image = ("@" + str(user) + " Mangotree image" + " Color: " + random.choice(COLOUR))

        if r.status_code == 200:
          media_id = r.json()['media_id']
          r = api.request('statuses/update', {'status': tweet_image, 'media_ids': media_id})
...
while True:
    try:
        iterator = api.request('statuses/filter', {'track':stringToTrack}).get_iterator()   
